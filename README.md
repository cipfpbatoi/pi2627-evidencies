...
En servidor, GitHub CLI ha d'estar instal·lat i autenticat. Si el servei no troba `gh`, configura `GH_BIN=/ruta/al/gh` en `.env`. Si vols usar un token específic per a `gh`, posa'l en `GH_TOKEN`; `GITHUB_TOKEN` queda reservat per a les crides del dashboard PI a l'API de GitHub. Per crear repositoris privats, convidar l'equip amb accés `write` i esborrar repositoris, els tokens han de poder administrar els repositoris de l'organització docent; amb tokens clàssics sol caldre `repo` i `delete_repo`. Si alguna invitació falla, el repositori es manté registrat i el dashboard identifica l'usuari que cal revisar o tornar a convidar des de «Gestionar accés».
